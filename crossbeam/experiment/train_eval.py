...
def train_step_bsize1(task, training_samples, all_values, model, optimizer, 
...
def singleproc_train_eval_loop(args, model, optimizer, eval_tasks, operations, constants, task_gen, trace_gen):
  pbar = tqdm(range(args.train_steps))
  eval_func = functools.partial(do_eval, max_search_weight=args.max_search_weight, beam_size=args.beam_size)
  for i in pbar:
    ...
    if isinstance(training_samples, list):
      loss = train_step_bsize1(t, training_samples, all_values, model, optimizer,
